...
def test_summary_cache_hits_memory(monkeypatch, tmp_path):
    ...
    s1 = _run(svc.summarize_context("file", "one.md", "concise"))
    s2 = _run(svc.summarize_context("file", "one.md", "concise"))
...
def test_summary_cache_hits_sqlite_across_instances(monkeypatch, tmp_path):
    ...
    assert _run(svc1.summarize_context("file", "one.md", "concise")) == "persisted-summary"
    assert calls["count"] == 1
    ...
    assert _run(svc2.summarize_context("file", "one.md", "concise")) == "persisted-summary"
...
def test_summary_cache_dedupes_inflight_requests(monkeypatch, tmp_path):
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        a = pool.submit(_run, svc.summarize_context("file", "one.md", "concise"))
        b = pool.submit(_run, svc.summarize_context("file", "one.md", "concise"))
...
def test_summary_background_shutdown_clears_async_runtime(monkeypatch, tmp_path):
    docs = _create_docs(tmp_path)
    data_path = tmp_path / "data"
    monkeypatch.setenv("DATA_PATH", str(data_path))
    ...
    svc = server.GameDocServer(docs_root=str(docs))
    svc._start_summary_background()

    assert svc._summary_background_loop is not None
    assert svc._summary_background_thread is not None
    assert svc._summary_background_thread.is_alive()

    _run(svc.shutdown())

    assert svc._summary_background_loop is None
    assert svc._summary_background_thread is None
    assert svc._summary_worker_task is None
    assert svc._summary_monitor_task is None
